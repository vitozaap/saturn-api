...
export class CompressorController {
    ...
    @ApiOperation({
        ...
        description: "Creates a PENDING_UPLOAD compression row and returns a presigned S3 URL to upload the source video to.",
    })
    ...
    @Get()
    @ApiOperation({ summary: "List compressions", description: "Returns every compression owned by the authenticated user." })
    ...
    @ApiOkResponse({ type: CompressionResponseDto, description: "Each SSE event's data is the current compression state." })
